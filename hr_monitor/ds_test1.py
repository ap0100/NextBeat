...
def print_bitmap(bitmap_bytes, width, height):
    for y in range(height):
        line = ""
        for x in range(width):
            byte_idx = (y * width + x) // 8
            bit = 7 - (x % 8)
            line += "#" if (bitmap_bytes[byte_idx] >> bit) & 1 else " "
        print(line)

def write_display(hr_str, spo2_str, status_str, ir_history=None):
    """
    ir_history: list of recent IR values (max 140) for waveform.
    """
    img = Image.new("RGB", (WIDTH, HEIGHT), (0, 0, 0))
    draw = ImageDraw.Draw(img)
    ...
    #Logo(bmp)
    img.paste(logo_h,(63,72)) 
    img.paste(logo_o,(59,105))

    #BPM val
    draw.text((108, 77), hr_str, font=small_font, fill=(0, 255, 120))

    #SpO_2 val
    draw.text((109, 115), spo2_str, font=small_font, fill=(80, 180, 255))
...
def fill_buffer():
    """Collect WINDOW_SAMPLES at the correct rate."""
    write_display("---", "---", "WARMING UP", None)
    ir_buf.clear(); red_buf.clear()
...
while True:
    ...
    if np.mean(recent) < FINGER_THRESH:
        if displayed_hr is not None:
            write_display("---", "---", "NO FINGER", None)
            displayed_hr = displayed_spo2 = None
            hr_history.clear(); spo2_history.clear()
            print("\nNo finger — waiting...")
        fill_buffer()
        samples_since_calc = 0
        print("Buffer ready. Monitoring.")
    ...
    # Calculate every SLIDE_SAMPLES new samples
    ...
    if not hr_history or not spo2_history:
        ...
        if smooth_hr != displayed_hr or smooth_spo2 != displayed_spo2:
            write_display(f"{smooth_hr} bpm", f"{smooth_spo2}%", status, ir_history=recent_ir)
            displayed_hr   = smooth_hr
            displayed_spo2 = smooth_spo2

        print(f"\rHR: {smooth_hr:3d} bpm | SpO2: {smooth_spo2:3d}% | [{status}]",
              end="", flush=True)
