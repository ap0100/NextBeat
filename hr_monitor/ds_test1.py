...
def write_display(hr_str, spo2_str, status_str, pred_hr_str=None, pred_spo2_str=None, ir_history=None):
    ...
    """#Title
    bb=draw.textbbox((0, 0), "NextBeat", font=txt_font)
    draw.text(((WIDTH - (bb[2]-bb[0])) // 2, 25), "NextBeat", font=txt_font, fill=(142, 245, 204))

    #Divide
    draw.line([(58,52),(180,52)],fill=(142, 245, 204),width=1)"""
    ...
    if ir_history and len(ir_history)>5:
        ...
        samples=list(ir_history)[-wave_width:]
        if samples:
            min_val=min(samples)
            max_val=max(samples)
            if max_val>min_val:
                y_vals = [(s-min_val) / (max_val-min_val)*wave_height for s in samples]
                points = []
                for i, y_norm in enumerate(y_vals):
                    x=wave_x_start+i
                    y=wave_y_bottom-y_norm
                    points.append((x, y))
                if len(points) >= 2:
                    draw.line(points, fill=(100, 255, 100), width=2)
            else:
                draw.line([(wave_x_start, wave_y_bottom), (wave_x_start+wave_width, wave_y_bottom)],
                          fill=(100,255,100), width=2)
    else:
        import math
        points=[]
        for x in range(140):
            y=180+10*math.sin(x*0.2)
            points.append((50+x, y))
        draw.line(points, fill=(100, 255, 100), width=2)
...
#--MODEL---------------
model=joblib.load('predict_30s.pkl')
...
def fill_buffer():
    write_display("---", "---", "WARMING UP", None, None, None)
    ir_buf.clear()
    red_buf.clear()
...
def send_email_alert(pred_hr, pred_spo2):
    try:
        msg=EmailMessage()
        msg.set_content(f"NextBeat Alert!\n\nPredicted HR: {pred_hr} bpm\nPredicted SpO2: {pred_spo2}%\n\nThis is a preemptive warning based on your recent heart rate trend.\nTime: {time.ctime()}")
...
while True:
    ...
    if np.mean(recent)<FINGER_THRESH:
        if displayed_hr is not None:
            write_display("---", "---", "NO FINGER", None, None, None)
            displayed_hr=displayed_spo2=None
            hr_history.clear()
            spo2_history.clear()
            live_hr.clear()
    ...
    if not hr_history or not spo2_history:
        write_display("---", "---", "MEASURING", None, None, None)
        ...
        if not baseline_established and len(live_hr)==LOGS:
            hr_mean=np.mean(list(live_hr))
            hr_std=np.std(list(live_hr))
            spo2_mean=np.mean(list(live_spo2))
            spo2_std=np.std(list(live_spo2))
            hr_threshold=hr_mean+20
            spo2_threshold=spo2_mean-5
            baseline_established=True
            print(f"\nBaseline set: HR={hr_mean:.0f}±{hr_std:.0f}, SpO2={spo2_mean:.0f}±{spo2_std:.0f}")
            print(f"Alert thresholds: HR>{hr_threshold:.0f} or SpO2<{spo2_threshold:.0f}")

        #Predict if we have enough history
        pred_hr=pred_spo2=None
        anomaly_status=status
        if len(live_hr)==LOGS:
            input_features=np.concatenate([list(live_hr), list(live_spo2)]).reshape(1, -1)
            pred=model.predict(input_features)[0]
            pred_hr=int(round(pred[0]))
            pred_spo2=int(round(pred[1]))

            #Pre‑emptive alert via SMS
        ...
        """#CSV logging
        csv_writer.writerow([time.time(), ir, red, smooth_hr, smooth_spo2, anomaly_status])
        csv_file.flush()"""
        
        if smooth_hr!=displayed_hr or smooth_spo2!=displayed_spo2:
            pred_hr_str=f"{pred_hr}" if pred_hr is not None else "--"
            pred_spo2_str=f"{pred_spo2}" if pred_spo2 is not None else "--"
            write_display(f"{smooth_hr} bpm", f"{smooth_spo2}%", anomaly_status, pred_hr_str, pred_spo2_str, recent_ir)
